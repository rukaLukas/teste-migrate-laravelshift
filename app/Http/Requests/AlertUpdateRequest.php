...
class AlertUpdateRequest extends FormRequest
{
    ...
    {
        $dateChild = $this->getDateChild('d/m/Y');
        return [
    ...
    private function validate() {        
        ...
        $validator->sometimes('cpf', 'nullable|cpf|unique:records,cpf', function ($input) use ($cpf) { // registro existente, nova visita/alerta        
            // TODO: se campo cpf nao vazio, verifica se o cpf informado é igual ao cpf do registro(com mesmo uuid)                        
            return !empty($input['cpf']) && $cpf != $input['cpf'];           
        });

        $validator->sometimes('suscard', 'nullable|size:8|unique:records,suscard', function ($input) use ($suscard) {
            return !empty($input['suscard']) && $suscard != $input['suscard'];   
        });        
    ...
    {
        $date = new Carbon();                                
        return $date->subYears(env('AGE_CHILD'))->format($format);
    }
